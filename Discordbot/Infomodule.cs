        ...
        public async Task FindUser([Remainder] string Username)
        {
            ...
            foreach (string[] matchid in Puuid_MatchID.Values)
            {
                ...
                if (diff != null)
                {
                    ...
                    // the champs played kda and other info 
                    foreach (var s in gameroots)
                    {
                        game = new GameViewModel();
                        var res = gameroots.Where(x => x.info.gameId == s.info.gameId).Select(x => x.info.participants.Select(f => f.win && f.puuid == playerPuuid).FirstOrDefault());
                        game.result = res.ElementAt(0) ? Result.Win : Result.Loss;
                        game.CS = (double)gameroots.SelectMany(x => x.info.participants.Select(f => f.totalMinionsKilled)).FirstOrDefault();
                        var seconds = gameroots.Select(x => x.info.gameDuration).FirstOrDefault();
                        var tspan = TimeSpan.FromSeconds(seconds);
                        game.Duration_Mins = (decimal)tspan.TotalMinutes;
                        game.CS_Min = (decimal)game.CS / game.Duration_Mins;
                        game.deaths = gameroots.Where(x => x.info.gameId == s.info.gameId).SelectMany(x => x.info.participants.Where(y => y.puuid == playerPuuid)).Select(x => x.deaths).FirstOrDefault();
                        game.kills = gameroots.Where(x => x.info.gameId == s.info.gameId).SelectMany(x => x.info.participants.Where(y => y.puuid == playerPuuid)).Select(x => x.kills).FirstOrDefault();
                        game.assist = gameroots.Where(x => x.info.gameId == s.info.gameId).SelectMany(x => x.info.participants.Where(y => y.puuid == playerPuuid)).Select(x => x.assists).FirstOrDefault();
                        view.Games.Add(game); 
                    }
                }
                keyfinder++;
